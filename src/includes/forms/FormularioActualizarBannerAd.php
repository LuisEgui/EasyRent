...
class FormularioActualizarBannerAd extends Formulario
{
    ...
    protected function generaCamposFormulario(&$datos)
    {
        ...
        $htmlErroresGlobales
        <fieldset>
            <legend>Actualizar banner de anuncio</legend>
            <div>
    ...
    protected function procesaFormulario(&$datos)
    {
        ...
        */
        ...
        $ok = $this->advertisementService->uploadBanner($nombre, $mimeType, $_GET['id']);

        if (!$ok) {
            $this->errores['archivo'] = 'Error al actualizar la imagen';
        }

        $imagen = $this->advertisementService->getBanner($_GET['id']);
        $fichero = "{$imagen->getId()}-{$imagen->getPath()}";
        $ruta = implode(DIRECTORY_SEPARATOR, [dirname(dirname(__FILE__)).'\img\ads', $fichero]);
